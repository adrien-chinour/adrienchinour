...
Même si quand on fait du web on n'a pas trop l'usage il peut être intéressant dans certains cas d'utiliser d'autres types de structures. Array est un mélange de différentes structure de données en permettant des opérations assez completes.
Mais d'un point de vue performance et compréhension c'est pas forcément le mieux qu'il soit.
...
Il s'agit de structures inclues dans la SPL qui sont activée par défaut.


Une alternative a cela est l'extension Data Structure qui comme son nom l'indique propose des structures de données (développé en C).
