...
Array, array ou array ? Oui c'est vrai que PHP n'est pas riche en structure de données mais voyons
ensemble un peu ce qu'il en est réellement ? 😊

<div style="text-align:center">
  <img src="les-structures-de-donnees-en-php/cover.jpg" width="100%" height="auto" />
</div>

## Les **array**

La structure de données par excellence en PHP c'est array, elle est _quasiment_ seule et on l'utilise comme liste,
table de hachage, pile, file, etc.
C'est plutôt utile notamment pour les débutants qui ne se retrouve pas étouffer par le typage fort de certains langages.
Mais c'est aussi ça qui en fait un langage détesté par de nombreux développeur. 👹

Voyons déjà le fonctionnement de cette structure de donnée _magique_... 🧐

_Array_ est en réalité un **dictionnaire ordonné**, avec un couple clé/valeur. Dans le cas où l'on ne défini pas de clé alors
PHP défini des clés numérique : 0, 1, 2, etc. Pour faire le lien entre cette clé et la valeur une table de hachage est créée.

Je ne vais pas rentrer dans les détails, je vous laisse lire <a rel="noreferrer noopener" target="_blank" href="http://nikic.github.io/2014/12/22/PHPs-new-hashtable-implementation.html">cet article</a> de Nikita Popov
qui explique en détail l'évolution entre **PHP 5** et **PHP 7** sur l'implémentation des tableaux en PHP.

> La documentation de Zend fourni aussi une explication très claire sur l'implémentation et l'API interne de PHP pour les arrays : <a rel="noreferrer noopener" target="_blank" href="https://www.zend.com/php-arrays">PHP Arrays</a>

Et le gros problème avec cette implémentation c'est que même si ça répond à un grand nombre de cas on a parfois des performances désastreuses
sur certaines applications juste parce que l'on utilise des functions PHP très peu performante.

Prenons le cas de <a rel="noreferrer noopener" target="_blank" href="https://www.php.net/manual/fr/function.array-shift.php">`array_shift`</a> qui permet d'extraire le premier élément d'un array.
De part sa structure interne un array avec des clés numérique va décaler chacune d'elles lors d'un `array_shift` ce qui cause une complexité de O(n). 😱
Sur des petits tableau l'opération n'est pas forcément couteuse mais lorsqu'on manipule des tableaux plus grands cela peut vraiment être un problème.
...
Des cas comme ça il y en a pas énormément et il est vrai uniquement sur des gros volumes de données mais on peut tout de même
se poser la question des alternatives, que ce soit pour le confort et la compréhension du code comme de la performance.
...
On comprend ce que cela fait mais honnêtement la lisibilité est pas top, on remarque vite que l'imbrication des fonctions peut vite poser problème.
...
On ne se simplifie pas forcément la vie, c'est même plus complexe puisque l'on mélange la manipulation de notre objet Collection avec les fonctions natives PHP.

L'intérêt de cette librairie est de fournir une implémentation utile pour doctrine dans le traitement des collections d'entités,
...
Comme on l'a vu au début PHP semble proposer une seule structure de donnée (_array_) mais depuis **PHP 7** nous avons la **SPL**...
...
> Je vous conseille cette vidéo qui explique un peu tout ce que je vais dire dans cette partie : <a rel="noreferrer noopener" target="_blank" href="https://www.youtube.com/watch?v=tX1jbqnjrR0">Les structures de données en PHP - Frédéric BOUCHERY - AFUP Day 2020 Nantes</a>

Mais le problème majeur de la SPL est que des opérations qui ne devraient pas être présente dans une structure de donnée soit possible.
On se retrouve avec SplStack qui permet de faire des shift/unshift ou des push/pop. 🤨

> TODO parler de SplFixedArray

Une autre extension qui malheureusement n'est pas présente de base dans PHP est l'extension Data Structure. Elle corrige tous les problèmes de la SPL en allant encore plus loin.
...
Pour vous aider à comprendre l'utilité voici un exemple qui reprend le problème de `array_shift` utile lorsqu'on utilise une file :
...
pour des cron ou des tâches asynchrones par exemple. _Array_ est indispensable à PHP, c'est simple et efficace, mais dans certains
cas **Data Structure** ou **Laravel Collection** rendent les projets bien plus sympa ! 🥳
